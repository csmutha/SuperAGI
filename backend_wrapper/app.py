...
SUPER_AGI_URL = 'http://backend:8001/'
...
@app.route('/api/agents/get/project/<id>', methods=['GET'])
def route__api_agents_get_project_id(id):
    response = requests.get(f'{SUPER_AGI_URL}agents/get/project/'+id)
...
@app.route('/api/agentexecutions/update/<id>', methods=['PUT'])
def route__api_agentexecutions_update_id(id):
    # body = {"PUT": "{\"status\":\"RUNNING\"}"}
    response = requests.put(f'{SUPER_AGI_URL}agentexecutions/update/'+id, json=request.get_json())
    try:
        return jsonify(response.json()), response.status_code
    except requests.exceptions.JSONDecodeError:
        return jsonify({"error": "Failed to parse response as JSON"}), 500
